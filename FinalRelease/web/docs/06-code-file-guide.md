...
| `pages/Settings.tsx` | 设置页和设置项二级详情。页面负责本地/云端设置同步、二次确认弹层、意见反馈、清理缓存、退出登录和注销账号入口。 | 后续可路由化为 `/settings/:section`，并为 rows 增加 action/route 字段。 |
...
## 2026-07-29 feedback module file update

- `services/feedbackApi.ts`: submits user feedback from settings to `POST /feedback`.
- `pages/Settings.tsx`: adds the "意见反馈" sheet under Help & About, including category, message, optional contact, and inline submit status.
- `components/AdminPanel.tsx` and `services/adminApi.ts`: show recent user feedback and allow admins to mark items as reviewed or closed.
- `server/src/modules/feedback.ts`, `server/src/types.ts`, `server/src/data/postgres.ts`, and `server/src/data/postgresMappers.ts`: add the `user_feedback` table, public submit API, and admin review APIs.
