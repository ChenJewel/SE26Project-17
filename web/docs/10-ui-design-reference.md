...
- [春日水面色卡](./ui-reference/xiaohongshu-five-notes/palette-spring-water.jpg)：当前全局颜色锚点，保留原图中的六个十六进制值。
- [首页春日水面参考图](./ui-reference/xiaohongshu-five-notes/home-spring-water-reference.jpg)：当前首页与饭卡详情的背景参考；运行时资源位于 `web/public/assets/home/spring-water-anime-bg.jpg`。
- [非首页樱花背景](./ui-reference/xiaohongshu-five-notes/non-home-frosted-background.jpg)：社区、消息、我的、发卡片和设置页的环境参考；运行时先模糊再覆盖冰白半透明层。
- [虹彩棱镜色卡](./ui-reference/xiaohongshu-five-notes/palette-rainbow-prism.jpg) 与 [扩展四季色卡](./ui-reference/xiaohongshu-five-notes/palette-extended-seasons.jpg)：用于标签、统计块和次级状态的色彩轮换。

### 当前色彩锚点

| 角色 | 原图颜色 | 使用范围 |
| --- | --- | --- |
| 主操作蓝 | `#17A1CF` | 主按钮、选中态、导航强调、关键链接 |
| 水面青 | `#81BAC2` | 次级强调、描边、浅色标签与玻璃环境色 |
| 冰白 | `#F9FAFB` | 页面表面、玻璃雾化底色、输入区域 |
| 玫瑰粉 | `#FADADA` | 轻提示、柔和标签、玻璃暖色反射 |
| 蜜桃红 | `#E2837D` | 次级状态、内容分类、暖色层级 |
| 西柚红 | `#DD4350` | 未读、危险操作、必须立即识别的状态 |

标签与小面积信息块允许使用以下扩展色：梦幻紫 `#9F99D1`、晶体蓝 `#86BADA`、仙女粉 `#DBAAD7`、珊瑚粉 `#F6BEB0`、奶蜜黄 `#FFE3B3`、糖果蓝 `#AFEBF3`、柔紫 `#B5A2D4`。这些颜色必须分组轮换，不能把整页或同一组件族全部染成一个色阶。

锚点不足时允许生成色相接近的临近色阶，例如更深的蓝色文字或更浅的青白表面。聊天详情继续使用暖白、薄荷绿和淡黄色，且必须通过局部主题变量隔离，不能污染消息列表和其他页面。
...
3. 色彩按页面语境变化。全局使用春日水面色卡及其临近色阶；聊天详情保留独立的暖黄绿色语境。
4. 真实图片负责氛围。优先让帖子媒体、头像、封面和场景图片形成色彩环境，不使用无意义的装饰色块或模糊光球。
...
- 首页与饭卡详情：共用春日花朵与蓝色水面背景，前景使用偏冰白但可透出环境色的毛玻璃；两处饭卡结构、雾度、描边和阴影保持一致。
- 社区：使用水面青、玫瑰粉与冰白组织状态层级；帖子真实媒体不加统一滤镜。
- 消息列表：跟随全局蓝粉色卡和冰白表面，不使用整页黄色主题。
- 聊天详情：使用暖白纹理背景、白色接收气泡、薄荷绿发送气泡和淡黄色系统消息；图片和语音消息不加重滤镜。
- 用户主页：使用主操作蓝、水面青和少量玫瑰粉建立资料层级，头像、封面和关系状态仍是第一视觉焦点。
