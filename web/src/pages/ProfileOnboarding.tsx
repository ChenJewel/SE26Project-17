    ...
    <main className="min-h-[100dvh] bg-[var(--page-bg)] text-[var(--text-main)]">
      <section className="mx-auto flex min-h-[100dvh] max-w-md flex-col px-5 pb-[calc(24px+env(safe-area-inset-bottom))] pt-10">
        <div>
          ...
          <p className="mt-2 text-sm font-semibold leading-6 text-[var(--text-muted)]">设置头像、昵称和可选偏好，保存后以后登录不会再出现这个页面。</p>
        </div>
            ...
            {visibleTags.map((tag) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-[72px] rounded-lg p-3 text-left ring-1 transition ${selected ? "bg-[rgba(209,228,221,0.82)] ring-[var(--pine)]" : "bg-white/82 ring-[var(--line-soft)]"}`}
                >
