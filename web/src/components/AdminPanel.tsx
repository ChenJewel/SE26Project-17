...
export function AdminPanel({ onClose }: { onClose: () => void }) {
  ...
  const createCode = async () => {
    ...
    try {
      ...
      await refresh();
      setNotice("邀请码已创建，请立刻复制保存，之后不会再次完整显示。");
    } catch (error) {
    ...
    <main className="fixed inset-0 z-[90] overflow-y-auto bg-[var(--page-bg)] text-[var(--text-main)]">
      <section className="mx-auto max-w-md px-4 pb-8 pt-4">
        <header className="page-header relative -mx-4 mb-4 flex h-16 items-center justify-center px-4">
          ...
          <button onClick={refresh} disabled={loading} className="absolute right-4 flex h-11 w-11 items-center justify-center rounded-lg text-[var(--pine)] disabled:opacity-60">
        ...
        <section className="mt-4 rounded-lg bg-white/86 p-4 shadow-sm ring-1 ring-[var(--line-soft)]">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <input value={label} onChange={(event) => setLabel(event.target.value)} className="col-span-2 h-11 rounded-lg bg-[var(--surface-soft)] px-3 text-sm font-bold outline-none ring-1 ring-[var(--line-soft)]" placeholder="邀请码名称" />
            <input value={maxUses} onChange={(event) => setMaxUses(Number(event.target.value) || 10)} className="h-11 rounded-lg bg-[var(--surface-soft)] px-3 text-sm font-bold outline-none ring-1 ring-[var(--line-soft)]" inputMode="numeric" placeholder="最多使用人数" />
            <input value={expiresInDays} onChange={(event) => setExpiresInDays(Number(event.target.value) || 14)} className="h-11 rounded-lg bg-[var(--surface-soft)] px-3 text-sm font-bold outline-none ring-1 ring-[var(--line-soft)]" inputMode="numeric" placeholder="有效天数" />
          </div>
              ...
              <div key={invitation.id} className="rounded-lg bg-[var(--surface-soft)] p-3 ring-1 ring-[var(--line-soft)]">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs font-bold text-[var(--text-muted)]">前缀 {invitation.codePrefix} · {invitation.usedCount}/{invitation.maxUses} · {formatDate(invitation.expiresAt)}</p>
                  </div>
        ...
        <section className="mt-4 rounded-lg bg-white/86 p-4 shadow-sm ring-1 ring-[var(--line-soft)]">
          ...
          <p className="mt-1 text-xs font-bold text-[var(--text-muted)]">待处理 {pendingReports.length} 条，最近显示前 20 条。</p>
          <div className="mt-3 space-y-2">
            {reports.slice(0, 20).map((report) => (
              <div key={report.id} className="rounded-lg bg-[var(--surface-soft)] p-3 ring-1 ring-[var(--line-soft)]">
                <p className="text-xs font-black text-[var(--pine)]">{report.targetType} · {report.status}</p>
                <p className="mt-1 text-sm font-bold text-[var(--text-main)]">{report.reason}</p>
                <p className="mt-1 truncate text-xs font-semibold text-[var(--text-muted)]">目标：{report.targetId}</p>
