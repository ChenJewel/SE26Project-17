import type { MealCard } from "../types.js";
...
export function filterHomeVisibleMealCards<T extends MealCardTimeFields>(cards: T[], now = new Date()) {
  return cards.filter((card) => isMealCardVisibleOnHome(card, now));
}

export function getMealCardScheduleScore(card: MealCardTimeFields, now = new Date()) {
  const scheduleDate = getMealCardScheduleDate(card, now);
  if (!scheduleDate) return 0.5;

  const hoursUntilMeal = (scheduleDate.getTime() - now.getTime()) / 36e5;
  if (hoursUntilMeal < 0) return 0.58;
  if (hoursUntilMeal <= 4) return 1;
  if (hoursUntilMeal <= 24) return 0.92;
  if (hoursUntilMeal <= 72) return 0.78;
  if (hoursUntilMeal <= 168) return 0.64;
  if (hoursUntilMeal <= 336) return 0.5;
  return 0.42;
}
...
function parseMealTime(value: string, anchor: Date) {
  ...
  if (monthDay) {
    ...
    const year = inferClosestYear(month, day, anchor);
    return buildDate(year, month, day, normalized);
  }
...
function parseClock(value: string) {
  ...
  if (colonClock) {
    return normalizeClock(Number(colonClock[1]), Number(colonClock[2]), value, true);
  }

  const cnClock = value.match(/(\d{1,2})\s*点\s*(半|(\d{1,2})\s*分?)?/);
  if (cnClock) {
    const minute = cnClock[2] === "半" ? 30 : Number(cnClock[3] ?? 0);
    return normalizeClock(Number(cnClock[1]), minute, value, true);
  }
...
function normalizeClock(hourInput: number, minuteInput: number, value: string, explicit: boolean) {
  ...
  if ((value.includes("下午") || value.includes("晚上") || value.includes("晚饭") || value.includes("今晚")) && hour < 12) {
    hour += 12;
  }
  if ((value.includes("中午") || value.includes("午饭")) && hour < 11) {
    hour += 12;
  }
