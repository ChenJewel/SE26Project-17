...
export const semanticTaxonomy: TaxonomyEntry[] = [
  ...
  location("dorm_area", "宿舍区", ["宿舍", "寝室", "宿舍区", "楼下", "宿舍楼下"]),
...
export function extractCanonicalTagMatches(text: string): CanonicalTagMatch[] {
  const normalized = normalizeRawToken(text);
  if (!normalized) return [];

  const matches = semanticTaxonomy
    .filter(
      (entry) =>
...
export function extractCanonicalTags(text: string) {
  return extractCanonicalTagMatches(text).map((match) => match.canonicalTag);
}

export function buildCanonicalTagMatches(draft: SemanticDraft): CanonicalTagMatch[] {
  const ruleMatches = uniqueMatches([
    ...extractCanonicalTagMatches(draft.text),
    ...(draft.rawTags ?? []).flatMap((tag) => extractCanonicalTagMatches(tag)),
  ]);
  ...
  const customMatches = (draft.rawTags ?? [])
    ...
    .filter((tag) => !ruleTags.has(resolveCanonicalTag(tag) ?? ""))
    .map((tag) => ({
      rawText: tag,
...
export function buildCanonicalTags(draft: SemanticDraft) {
  return buildCanonicalTagMatches(draft).map((match) => match.canonicalTag);
}
...
export function normalizeToCanonicalTag(value: string) {
  const normalized = normalizeRawToken(value);
  return aliases.get(normalized)?.tag ?? normalized;
}

export function resolveCanonicalTag(value: string) {
  return aliases.get(normalizeRawToken(value))?.tag;
}
...
export function normalizeSemanticTokens(values: Array<string | undefined>, options: { includeCustom?: boolean } = {}) {
  return unique(
    values.flatMap((value) => {
      const canonical = resolveCanonicalTag(value ?? "");
